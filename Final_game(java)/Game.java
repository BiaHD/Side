...
class Game extends JFrame implements MouseListener{
    private int width=700,height=750,mapRow=9,mapCol=9; //width:視窗寬、height:視窗長、mapRow:地圖row、mapCol:地圖col
    ...
    public void mouseClicked(MouseEvent e){
        ...
        if(command[0].equals("r")){
            ...
            if(e.getButton()==MouseEvent.BUTTON1){
            ...
            }else if(buttonIsPress[row][col] && button[row][col].getBackground().equals(Color.BLUE) && e.getButton()==MouseEvent.BUTTON2){
                /*取消標記的炸彈按紐。*/

                buttonIsPress[row][col]=false; //取消按壓。
                button[row][col].setBackground(Color.WHITE); //設定按鈕背景顏色。
                button[row][col].setText(""); //印出數字
                bombCount++; //炸彈數。
                bombnumber.setText("目前炸彈數："+bombCount);
            }else if(e.getButton()==MouseEvent.BUTTON3 && !buttonIsPress[row][col]){
                /*標記炸彈。並判斷是否結束遊戲。*/

                ((JButton)e.getSource()).setBackground(Color.BLUE); //設定按鈕背景顏色。
                buttonIsPress[row][col]=true; //設定按鈕為按過。
                bombCount--; //炸彈數。
                bombnumber.setText("目前炸彈數："+bombCount);
                ...
                if(bombCount==0){
                    ...
                    for(int i=0;i<mapRow;i++){
                        for(int j=0;j<mapCol;j++){
                            if(map[i][j]==1 && !buttonIsPress[i][j]) endGame=false;
                        }
                    }
                    if(endGame){
                        timeContinue=0; //時間停止計時。
